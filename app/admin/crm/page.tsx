...
import { Plus, Search, Edit2, Trash2, Phone, Mail, MapPin, X, Save, Loader2, User, Upload } from 'lucide-react'
  ...
  return (
    <div className="p-6 md:p-8 pt-16 md:pt-8 h-full">
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-extrabold text-gray-900">CRM — Contacts</h1><p className="text-gray-500 text-sm">{contacts.length} total</p></div>
        <div className="flex gap-3">
          ...
          <button onClick={() => { setForm(empty); setEditing(false); setShowForm(true) }} className="flex items-center gap-2 text-white font-semibold px-4 py-2.5 rounded-xl shadow-md" style={{ background:'#185FA5' }}>
      ...
      <div className="flex gap-5 h-[calc(100vh-240px)]">
        <div className={`flex flex-col overflow-hidden rounded-2xl bg-white shadow-sm border border-gray-100 ${selected ? 'w-2/5' : 'w-full'}`}>
          <div className="overflow-y-auto flex-1">
            {loading ? <div className="flex items-center justify-center h-32"><Loader2 size={24} className="animate-spin" style={{ color:'#185FA5' }} /></div>
              ...
              : filtered.map(c => (
                <div key={c.id} onClick={() => setSelected(c === selected ? null : c)}
                  ...
                  style={{ borderLeftColor: selected?.id === c.id ? '#185FA5' : 'transparent' }}>
                  <div className="w-10 h-10 rounded-full flex items-center justify-center text-white font-bold text-sm flex-shrink-0"
                    style={{ background:'linear-gradient(135deg,#4AADE0,#185FA5)' }}>{c.first_name[0]}{c.last_name[0]}</div>
                  <div className="flex-1 min-w-0">
                    ...
                    {c.company_name && <div className="text-xs truncate" style={{ color:'#185FA5' }}>{c.company_name}</div>}
                  </div>
            ...
            <div className="p-6">
              <div className="flex items-start justify-between mb-6">
                <div className="flex items-center gap-4">
                  <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-white font-extrabold text-lg" style={{ background:'linear-gradient(135deg,#4AADE0,#185FA5)' }}>{selected.first_name[0]}{selected.last_name[0]}</div>
                  <div>
                    <h2 className="text-xl font-bold text-gray-900">{selected.first_name} {selected.last_name}</h2>
                    {selected.company_name && <p className="text-sm" style={{ color:'#185FA5' }}>{selected.company_name}</p>}
              ...
              <div className="space-y-2">
                {selected.phone && <a href={`tel:${selected.phone}`} className="flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 transition-colors"><Phone size={15} style={{ color:'#185FA5' }} /><span className="text-gray-700 text-sm">{formatPhone(selected.phone)}</span></a>}
                {selected.email && <a href={`mailto:${selected.email}`} className="flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 transition-colors"><Mail size={15} style={{ color:'#185FA5' }} /><span className="text-gray-700 text-sm">{selected.email}</span></a>}
                {selected.address && <div className="flex items-start gap-3 p-3 rounded-xl"><MapPin size={15} className="mt-0.5" style={{ color:'#185FA5' }} /><div className="text-sm text-gray-700"><div>{selected.address}</div><div>{[selected.city,selected.state,selected.zip].filter(Boolean).join(', ')}</div></div></div>}
              </div>
              ...
              <div className="mt-5 flex gap-3">
                <button onClick={() => { setForm({...selected}); setEditing(true); setShowForm(true) }} className="flex-1 flex items-center justify-center gap-2 text-white font-semibold py-2.5 rounded-xl" style={{ background:'#185FA5' }}><Edit2 size={14} />Edit</button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="px-6 pb-6 flex gap-3">
              <button onClick={save} disabled={saving} className="flex-1 flex items-center justify-center gap-2 text-white font-bold py-3 rounded-xl disabled:opacity-60" style={{ background:'#185FA5' }}>
